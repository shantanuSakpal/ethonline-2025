...
import { DialogueEditDCA } from "@/components/co-pilot/dialogue-edit-dca";
import { FREQUENCIES } from "@/components/co-pilot/select-frequency";
import { Spinner } from "@/components/ui/spinner";
import { DialogueDcaFailedDetails } from "@/components/co-pilot/dialogue-dca-failed-details";
        ...
        {activeDCAs.map((dca) => {
          ...
          return (
            <TableRow key={uniqueKey}>
              ...
              >
                {FREQUENCIES.find(
                  (freq) => freq.value === purchaseIntervalHuman
                )?.label || purchaseIntervalHuman}
              </TableCell>
              <TableCell
                style={{
                  fontFamily:
              ...
              <TableCell>
                <div className="flex flex-row items-center justify-end gap-2">
                  ...
                  {active ? (
                    <Button size="sm" onClick={() => handleDisableDCA(dca._id)}>
                      <Pause className="w-4 h-4" />
                    </Button>
                  ) : (
                    <Button size="sm" onClick={() => handleEnableDCA(dca._id)}>
                  ...
                  <Button
                    variant="destructive"
                    size="sm"
                    onClick={() => handleDeleteDCA(dca._id)}
                  >
...
export const ActiveDcas: React.FC = () => {
  ...
  const handleUpdatedDCA = useCallback(
    async (updatedDCA: DCA) => {
      try {
        const updatedDCAs = [...activeDCAs];
        const index = updatedDCAs.findIndex(
          (dca) => dca._id === updatedDCA._id
        );
